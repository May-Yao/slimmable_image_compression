...
class Darknet_tol12(nn.Module):
    """YOLOv3 object detection model"""

    def __init__(self, config_path):
        super(Darknet_tol12, self).__init__()
        self.module_defs = parse_model_config(config_path)
        self.hyperparams, self.module_list = create_modules(self.module_defs)
...
class Darknetl12_tolast(nn.Module):
    """YOLOv3 object detection model"""

    def __init__(self, config_path):
        super(Darknetl12_tolast, self).__init__()
        self.module_defs = parse_model_config(config_path)
        self.hyperparams, self.module_list = create_modules(self.module_defs)
        self.yolo_layers = [layer[0] for layer in self.module_list if isinstance(layer[0], YOLOLayer)]
        self.seen = 0
...
def load_model(model_path, weights_path=None):
    ...
    device = "cuda:0"
    model = Darknet_tol12(model_path)
...
def load_ori_model(model_path, weights_path=None):
    """Loads the yolo model from file.
...
def load_test_model(model_path, weights_path=None):
    """Loads the yolo model from file.
    ...
    device = "cuda:0"
    # model = Darknet(model_path)
    model = Darknetl12_tolast(model_path)
